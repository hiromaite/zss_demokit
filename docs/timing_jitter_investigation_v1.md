...
4. GUI recording CSV と CLI timing probe を同じ session 条件で比較する
5. `sequence gap` と `status_flags` の overrun bit の有無を優先観測値として扱う
...
2. current CSV の `inter_arrival_ms` semantics を `host-handled jitter` として明示する
...
2. `wired_receive_path_probe.py` による poll sensitivity measurement
