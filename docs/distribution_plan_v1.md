...
1. `codex/project-organization-cleanup` と `codex/release-readiness-prep` の内容を
   review し、配布対象 branch へ統合する。
...
- current release-readiness branch の差分を review 可能な単位にする。
- PR を作る場合は、`codex/release-readiness-prep` を base branch へ向ける。
- 配布対象にする branch を決める。通常は `main` へ merge してから tag を打つ。

### Step 2: Local Preflight

```sh
.venv_gui_prototype/bin/python tools/release_readiness_check.py
