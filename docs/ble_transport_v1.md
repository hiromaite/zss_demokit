...
| `TELEMETRY_BATCH_CHARACTERISTIC_UUID` | `8B1F1001-5C4B-47C1-A742-9D6617B10004` | `NOTIFY` | Batch telemetry for 10 ms sample streams |
...
- `telemetry_field_bits` must only advertise fields physically carried by this 32 byte packet
- BLE v1 single-sample telemetry currently carries base 2ch plus `differential_pressure_selected_pa`; raw SDP channels are carried by batch schema v2, and service visibility diagnostics remain omitted from BLE
- Firmware may rate-limit this legacy-compatible notify while batch telemetry carries the full 10 ms sample stream

## 7.4 Telemetry Batch Packet v2

### 7.4.1 Carrier

- Characteristic: `TELEMETRY_BATCH_CHARACTERISTIC_UUID`
- Operation: `NOTIFY`
- Packet size: `16 + 28 * sample_count` bytes
- Maximum current sample count: `5`
- Firmware requests preferred MTU `185` so the current maximum packet size `156` fits within a typical ATT payload budget of `182` bytes
...
| `0` | `1` | `uint8` | `protocol_version_major` | v1 = `1` |
| `1` | `1` | `uint8` | `protocol_version_minor` | v1 = `0` |
| `2` | `1` | `uint8` | `batch_schema_version` | current = `2`; GUI keeps decode compatibility with schema `1` |
| `3` | `1` | `uint8` | `sample_count` | Number of repeated samples |
| `4` | `4` | `uint32` | `first_sequence` | Sequence of sample `0` |
| `8` | `4` | `uint32` | `first_sample_tick_us` | Device `micros()` tick for sample `0` |
| `12` | `2` | `uint16` | `nominal_sample_period_ms` | Target sample period |
| `14` | `2` | `uint16` | `telemetry_field_bits` | Fields present in each repeated sample |

Repeated sample record:

| Relative Offset | Size | Type | Field | Notes |
| :--- | :--- | :--- | :--- | :--- |
| `0` | `4` | `uint32` | `sample_tick_delta_us` | Delta from `first_sample_tick_us` |
| `4` | `4` | `uint32` | `status_flags` | Snapshot for this sample |
...
| `20` | `4` | `float32` | `differential_pressure_low_range_pa` | Raw `SDP810`; NaN when unavailable |
| `24` | `4` | `float32` | `differential_pressure_high_range_pa` | Raw `SDP811`; NaN when unavailable |

Interpretation:

- GUI expands each repeated record into a normal `TelemetryPoint`
- `sequence = first_sequence + sample_index`
- `device_sample_tick_us = first_sample_tick_us + sample_tick_delta_us`
- The same logical field names are used as legacy telemetry
- `telemetry_field_bits` in the batch header is the union of fields available in the batch; GUI treats non-finite raw SDP values as unavailable for that sample
- Raw SDP diagnostic fields are intentionally available only on batch telemetry, not on the 32 byte legacy BLE single-sample/status packets
...
- `telemetry_field_bits` follows the same BLE v1 single-sample mask as telemetry notify
- raw SDP channels and service visibility diagnostics must not be advertised in this packet unless the byte layout is extended in a future schema
...
| `4` | `telemetry_batch_supported` | Batch telemetry characteristic is available |
