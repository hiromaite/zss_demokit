...
| `GUI-VAL-029` | Plot pause and series visibility smoke | acquisition / recording を止めずに plot freeze と series on/off ができる | `PASS` | 2026-05-03 に `tools/gui_plot_controls_smoke.py` を QSettings isolated / explicit time span の no-device release gate として再確認。pause 中も buffer / recording rows が進み、resume と series visibility toggle が成立することを確認 |
