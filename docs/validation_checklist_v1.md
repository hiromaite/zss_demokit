...
| `GUI-VAL-028` | Cross-resolution layout smoke | Windows / low-resolution / narrow window で left/right scroll、plot height、log height、splitter が破綻しない | `PASS` | `tools/gui_layout_smoke.py` で `1480x1100` / `1480x940` / `1366x768` / `1280x720` / `1024x700` を確認。left/right horizontal scroll なし、metric cards 1 row、metric / toolbar compact height、plot splitter minimum `600px` and tall-window growth、splitter resize response を確認 |
| `GUI-VAL-029` | Plot pause and series visibility smoke | acquisition / recording を止めずに plot freeze と series on/off ができる | `PASS` | `tools/gui_plot_controls_smoke.py` で pause 中も buffer / recording rows が進み、resume と series visibility toggle が成立することを確認 |
| `GUI-VAL-030` | Engineering tools navigation smoke | Settings が肥大化せず、Flow Verification / Characterization / diagnostics へ迷わず到達できる | `PASS` | `tools/gui_engineering_tools_smoke.py` で Settings nav に `Engineering / Tools` が追加され、O2 calibration は `Device`、Flow Verification / Characterization / diagnostics は tools page から到達できることを確認 |
...
| `GUI-VAL-040` | Release readiness metadata / document smoke | release packaging metadata、icon assets、PyInstaller spec、release docs の整合を機械的に確認できる | `PASS` | `tools/release_readiness_check.py` で `APP_VERSION=1.0.0`、`zss_demokit_gui_win64_1_0_0`、icon、spec hiddenimports、`README.md`、`AGENTS.md`、`docs/project_status_v1.md`、`docs/distribution_plan_v1.md`、`docs/release_notes_v1_0_0.md`、Windows smoke checklist 参照を確認 |
| `GUI-VAL-041` | O2 output filter / zero-reference smoke | O2 zero reference、filter presets、settings migration、clamp diagnostics、filtered plot output が成立する | `PASS` | `tools/o2_filter_smoke.py` と `tools/gui_o2_filter_smoke.py` で voltage conversion zero anchor、disabled filter identity、Savitzky-Golay / centered Gaussian presets、stale calibration migration、custom parameter editing、clamp diagnostic detail、GUI filter controls、filtered O2 plot を確認 |
| `GUI-VAL-042` | Startup behavior and O2 2.55 V zero-reference polish smoke | Settings で launcher selection mode と BLE startup mode を切り替えられ、O2 0%基準の既定値が `2.55 V` へ移行する | `PASS` | `tools/gui_startup_mode_smoke.py` で default launcher route、BLE startup route、SettingsDialog startup control を確認。`tools/gui_o2_filter_smoke.py` で legacy `0.0 V` / `2.5 V` default anchor を初回 `2.55 V` へ移行し、移行後の explicit custom zero reference は保持されることを確認 |
| `GUI-VAL-043` | Windows release-candidate packaged smoke | latest `main` release candidate が Windows packaged app として起動し、blocking issue なく操作できる | `PASS` | user による Windows 11 Pro 実機確認で、最新 commit の packaged app 起動テストが OK。これを受けて `1.0.0` metadata / release notes / distribution docs を最終化 |
...
| `FW-VAL-018` | Pump / heater safety interlock regression | pump OFF 時に heater が OFF へ落ち、pump OFF 中の heater ON 指令が拒否される | `PASS` | `tools/command_processor_smoke.py` で heater単独ON拒否、pump ON後heater許可、pump OFF時heater強制OFF、invalid argument rejection を host-side regression として確認 |
...
| `FW-VAL-020` | Device-side 10 ms cadence | device sample interval が nominal `10 ms` 近傍に維持される | `PASS` | `codex/fw-acquisition-scheduler` 実機 test で sequence gap `0`, device interval `mean=10.000 ms`, `min=9.995 ms`, `p95=10.001 ms`, `max=10.004 ms` を確認 |
...
| `FW-VAL-023` | Acquisition cadence probe | full ADS read / dual-SDP read により wired sample frame が `10 ms` 近傍に戻る | `PASS` | faulty SDP810 交換後、ADS ch0/ch1/ch2 を毎 sample 取得に戻しても acquisition `mean=5.464 ms`, slow acquisition `0` を確認 |
| `FW-VAL-024` | Sub-ms scheduler jitter tuning | device-side sample tick jitter を acquisition ではなく scheduler/task 側で評価する | `PASS` | deadline-aware cooperative wait により device sample jitter `max_abs=5 us`, scheduler lateness `max=0.006 ms` を確認。BLE/batch 条件では再検証する |
...
| `INT-VAL-019` | Optional diagnostic availability UX | BLE / Wired で提供される diagnostic fields の差が GUI と CSV 上で誤解なく扱われる | `PASS` | Device Status に raw SDP、service voltage、BLE batch availability を追加し、`tools/gui_observability_smoke.py` で transport差の表示を確認。CSV raw columns は既存の raw-SDP batch parity で確認済み |
...
- SDP810 個体交換後、`SDP frontend initialized: low=1 high=1` と capabilities `telemetry_field_bits=123` を確認し、low/high raw の両方が wired payload へ戻った
- 情報量低下になっていた ADS1115 ch0/ch1 stagger を戻し、`MeasurementCore` は全 ADS channel を毎 sample 取得する構成へ戻した
- deadline-aware cooperative wait を追加し、deadline 直前は BLE / serial / LED などの非クリティカル処理より sample を優先するよう変更した
