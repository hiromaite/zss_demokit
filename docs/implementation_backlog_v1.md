...
- current focus は feature extension bundle の継続実装であり、現在は `EXT-004` dual-SDP differential pressure PoC の compile-safe frontend と実機 bring-up を進めている
...
5. `EXT-005` Flow telemetry integration `IN_PROGRESS`
...
- `PlotController` と `RecordingController` は、field が存在する場合はそれを優先して flow rate を算出し、存在しない場合は旧 `flow_sensor_voltage_v` placeholder へ fallback する
- `python3.12 -m compileall gui_prototype/src/protocol_constants.py gui_prototype/src/mock_backend.py gui_prototype/src/controllers.py` と `tools/protocol_fixture_smoke.py` は通過済み
- 次は firmware transport payload と GUI decoder に `differential_pressure_selected_pa` を実際に追加する
