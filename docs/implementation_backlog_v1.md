...
- PWM 化後の `pio run` と `tools/protocol_fixture_smoke.py` は通過済み
- 直近の upload / wired 実機 regression は Bundle A 初版時点で通過済みだが、NMP03 向け PWM 化後の実ポンプ確認は device reconnect 待ち
- 残件は physical button と LED visual pattern の実機手動確認
