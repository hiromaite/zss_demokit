...
- current focus は feature extension bundle の継続実装であり、現在は `EXT-003` O2 1-cell display and calibration の first pass と operator validation を進めている
...
3. `EXT-003` O2 1-cell display and calibration `IN_PROGRESS`
...
- 残件は ambient-air 実機 validation と、必要なら polarity reverse の有効化判断である
