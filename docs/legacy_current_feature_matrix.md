...
| Pump control via physical on-device button | Yes | Yes | 現行 firmware に local button controller を再導入し、Bundle A close-out で受け入れ済み |
| Internal ADC measurement: internal voltage | Yes | Partial | 現行 firmware / GUI / CSV に optional diagnostic field として配線済み。ただし current board config では internal ADC pin を無効化しており live value は unavailable |
...
| External ADC measurement: Zirconia Ip voltage | Yes | Yes | 現行 firmware で ADS1115 ch0 読み取りを復活。現在は LED 制御向け local measurement |
...
| Rich WS2812 status LED behavior | Yes | Yes | 現行 firmware に WS2812 state machine を再導入し、Bundle A close-out で受け入れ済み |
| Voltage-target aware LED behavior | Yes | Yes | 現行 firmware に `zirconia_ip_voltage_v` ベースの target logic を実装済み |
| BLE advertising / connected LED patterns | Yes | Yes | 現行 firmware に advertising / connected pattern を再導入し、Bundle A close-out で受け入れ済み |
...
| Real-time value: Internal Voltage | Yes | Partial | 現行 GUI の `Device Status` 内へ wired-first diagnostics として追加済み。current board config では live value unavailable |
| Real-time value: Zirconia Ip Voltage | Yes | Yes | 現行 GUI の `Device Status` 内へ wired-first diagnostics として追加済み |
...
| Editable calibration persistence | Yes | Yes | 現行は O2 1-cell ambient-air anchor と timestamp を `QSettings` に保存 |
...
| Recording-active visual emphasis | Yes | Yes | 現行 GUI は panel accent, active badge, recording toggle visual treatment を持つ |
...
- firmware / GUI 共通
  - `Internal Voltage` の current board config での live availability
  - `ZSS 2-cell raw value`
- GUI / operator feature
  - `O2 concentration (2-cell)` の表示
